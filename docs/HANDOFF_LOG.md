...
- CAUSA RAÍZ (reproducida con fixture sintético): con una viga LARGA que es la ÚNICA de su fila (una viga por renglón, contorno en LINEs largas — excluidas del anclaje por el filtro >30% diagonal), la retícula de bandas de `dxf-view-segmentation.ts` no tiene un segundo código en la banda ⇒ `lastSegmentReach` cae al `attachRadius` (~7.5% de la diagonal) ⇒ los tramos intermedios quedan `far` en clusters sueltos sin código (vistas `unknown`) y la vista de la viga queda RECORTADA al vecindario del código. El rescate F8E exigía `bbox de la vista + linkRadius`, que no cubre lo que la segmentación ya perdió ⇒ solo sobrevivían las barras junto al código.
- FIX (`dxf-beam-detail-assembly.ts`, sin tocar la segmentación): el rescate de textos longitudinales usa ahora el TERRITORIO de la fila/columna de la vista (`inBeamRowTerritory`): franja de la banda propia extendida por el eje de la secuencia hasta el inicio de la SIGUIENTE vista de viga (convención de los despieces: el territorio de un detalle va desde su código hasta el siguiente); sin vecino, hasta el borde de la lámina. Jamás invade: lo asignado FIRMEMENTE a otra vista de viga sigue excluido, contención estricta en bbox ajeno sigue vetada, y las vistas `table`/`title_block` jamás aportan barras. Orden por eje dominante y dedupe SOLO por token+posición intactos (F8E).
- DIAGNÓSTICO DE EXCLUSIÓN (nuevo, F8F.1): `BeamDetail.longitudinalExclusions` registra TODO texto longitudinal del DXF que no aparece en superior/inferior del detalle, con razón tipada (`fuera_de_vista`/`asignada_a_otra_vista`/`deduplicada`/`no_parseable`/`banda_ambigua`/`descartada_por_title_block`/`descartada_por_radio`/`descartada_por_bbox`) + `LONGITUDINAL_EXCLUSION_REASON_LABEL`. Post-pass en `assembleBeamDetails`: exclusiones cuyo texto SÍ quedó leído en otro detalle se re-etiquetan `asignada_a_otra_vista` ("leída en el detalle de X"); las restantes con riesgo real de pérdida generan advertencia visible. Panel "Ver detalle" (sección Evidencia): campo "Textos long. no incluidos" + lista del diagnóstico. Nada desaparece en silencio.
- CONTRATO F8F INTACTO: cantidad 1 por aparición textual DXF (editable), sin depender de marcadores, primer dígito jamás cantidad, overrides estructurados al dispatch, estribos solo con match o decisión humana (F8D), sin clasificar solo con decisión.
- GOLDEN TEST obligatorio (`f8f1-golden-beam-sequence.test.ts`, +18): fixture sintético por código (jamás DXF real) que REPRODUCE el layout de producción (viga larga única en su fila + segunda fila para la retícula). Exige: superior EXACTO `6#6330/6#6840/6#6900/6#6350` (desde `6#6 33 L`/`6#635` normalizados), inferior EXACTO `6#6440/6#6840/6#6730/6#6400`, dispatch con estribo match = 9 líneas (4+4+1), sensibilidad (si falta un tramo intermedio el conteo lo detecta), dedupe superpuesto con diagnóstico, 6#6840 arriba y abajo = 2 líneas, ambiguo ⇒ sin_clasificar + `banda_ambigua`, acciones asignar superior/inferior ⇒ 10 líneas (5+4 / 4+5), mismatch de estribo ⇒ 8 longitudinales jamás 0, invariante "visible o excluido con razón" por detalle.
- Tests: 3118→3136 (+18); suite steel 548/0; suite completa 3136/0 (42 skipped gated); typecheck 0; lint 0.
- Restricciones: sin DB/Supabase/RLS/migraciones/storage/env/APIs/dependencias nuevas; sin navegación global; sin P2D; sin archivos reales en Git; flag `STEEL_OPS_UIX_PREVIEW` intacto; F1 única calculadora; nada se autoaprueba; sin cambio del contrato de cantidad F8F; sin rediseño de UI (solo diagnóstico en Evidencia).

---

## 2026-07-07 - STEEL_OPS_F8F_LONGITUDINAL_OCCURRENCE_QUANTITY_ACTIONABLE_REVIEW - EN RAMA + PR - Fable

- Rama `feature/steel-ops-f8f-longitudinal-occurrence-review` (base `origin/main = ee59405`, post F8E #69). PR `fix(steel): use textual occurrence quantity for DXF longitudinal bars`. Problema conceptual de producción (panel VC-EJE-3): el sistema BLOQUEABA barras longitudinales superiores/inferiores por falta de "conteo gráfico confiable de marcadores" — incorrecto para el flujo real de cantidades de la usuaria.
- A Contrato de cantidad corregido (`dxf-beam-detail-assembly.ts`): `LongitudinalBarReading` ahora lleva `quantity` (=1), `quantityMode` (`textual_occurrence|graphic_marker|manual|unresolved`), `quantitySource` ("aparición textual DXF"), `markerEvidence`/`markerConfidence` (los marcadores pasan a EVIDENCIA de apoyo, jamás requisito) y `readingId` estable para decisiones. Cada texto longitudinal válido del DXF = UNA línea computable con cantidad 1 (regla operativa, NO inventada). El primer dígito del texto jamás es cantidad: el dispatch manda `originalDescription: "6#6350"` + overrides estructurados `manualQuantity: '1'`/`manualCutLengthM: '3.5'`/`manualBarNumber: 6` para que F1 no reinterprete `6#6350` como 6 unidades. `computeManualLine` (manual-takeoff.ts) aplica los overrides SOLO como inputs (F1 sigue única calculadora); `ManualLineRecord += manualQuantity/manualCutLengthM`; `ManualLineEvidence += quantityMode/quantitySource`.
- B Panel accionable (`beam-detail-review-panel.tsx`): cada barra superior/inferior con cantidad editable (input), longitud editable (input), "Aceptar línea" y "Marcar para revisión"; cada barra sin clasificar además con "Asignar a superior/inferior", "Aceptar sin clasificar" y "Mantener sin clasificar". Muestra "Cantidad: 1 por aparición textual DXF", "Marcadores gráficos: apoyo visual N / no confiables / no disponibles" y "Estado: computable, pendiente de aprobación". Las decisiones (`BeamLongitudinalDecisions` por `readingId`) viajan al dispatch; el envío sigue siendo el único "Enviar al takeoff".
- C Dispatch corregido (`buildBeamTakeoffDispatch` acepta `decisions`): 2 sup + 2 sin clasificar asignadas a inferior + estribo match ⇒ "Se enviarán 5 línea(s): 2 superior, 2 inferior, 1 estribo."; 4+4+1 ⇒ 9 líneas; mismatch de estribo sin decisión ⇒ envía longitudinales y bloquea SOLO el estribo; ambiguous jamás entra; `unclassifiedCount` nuevo en el preview cuando la usuaria acepta sin clasificar. Bloqueos reales que sí excluyen: longitud/diámetro no parseables, cantidad editada inválida, barra marcada para revisión, banda sin decisión.
- D Advertencias: "sin conteo gráfico confiable" DEJÓ de ser bloqueante/statusReason; ahora es informativa ("Marcadores gráficos no confiables o no disponibles; se usa cantidad 1 por aparición textual DXF. Puedes editar la cantidad." — `LONGITUDINAL_NO_MARKER_WARNING`). Estribos conservan la lógica F8D completa (match/mismatch/unverified/ambiguous + decisión humana).
- E Excel: CANT=1 (J), SON=1 (K), DESCRIPCION="6#6350" (D), Ø/código #6 (E/F), LONG CORTE 3.50 (I) vía overrides en `computeManualLine`; fórmulas VC-VERF intactas (N=I*J*K recalcula); EVIDENCIAS += columna "modo cantidad" (etiquetas es: "cantidad por aparición textual DXF"/"cantidad editada por la usuaria"/...); la observación de cada línea declara la fuente de cantidad y los marcadores como apoyo.
- F Copy: panel e intake dicen "cada texto longitudinal superior/inferior se envía como una línea computable con cantidad 1 por aparición textual DXF, editable por la usuaria; los marcadores gráficos se usan como evidencia de apoyo".
- Tests: +20 nuevos (`f8f-longitudinal-occurrence.test.ts`: contrato A con caso real 6#6 35 L/33 L/40 L/44 L, override F1 anti-"6 unidades", decisiones B/C, 5 y 9 líneas, estribos, Excel CANT/fórmulas/evidencia, estáticos UI/copy) + actualizados F8E/F8C/Excel al contrato nuevo. Suite completa 3118/0 (42 skipped gated); typecheck 0; lint 0.
- Restricciones: sin DB/Supabase/RLS/migraciones/storage/subidas/APIs/keys/dependencias nuevas; sin archivos reales en Git; flag `STEEL_OPS_UIX_PREVIEW` intacto; sin tocar nav global/ACCESS_MODULES/NAV_ITEMS; F1 única calculadora (overrides = inputs estructurados, no fórmulas nuevas); nada se autoaprueba; cantidad 1 por ocurrencia textual = regla operativa declarada, jamás inventada.

---

## 2026-07-06 - STEEL_OPS_F8E_LONGITUDINAL_SEQUENCE_TAKEOFF_DISPATCH - EN RAMA + PR - Fable

- Rama `feature/steel-ops-f8e-longitudinal-sequence-takeoff` (base `origin/main = 12f2b86`, post F8D #68). PR `feat(steel): complete DXF longitudinal beam dispatch`. Problema real de producción con "2504-010A R1 Refuerzo Vigas de Cimentación": el refuerzo superior/inferior solo leía el primer y el último texto de la línea (se saltaba tramos intermedios tras traslapos/cambios de tramo) y "Enviar al takeoff" terminaba mandando casi solo estribos.
- A Secuencia longitudinal COMPLETA (`dxf-beam-detail-assembly.ts`): (1) RESCATE por bbox de vista — los textos longitudinales que la asignación estricta F8D dejaba en franja ambigua o en cluster suelto pero que caen dentro del bbox de LA vista de la viga (margen `linkRadius`; contención ESTRICTA sin margen para vistas ajenas ⇒ sin fugas al vecino) se recuperan con warning y `rescuedLongitudinalTextCount` visible; lo asignado firmemente a OTRA vista de viga jamás se toca. (2) Orden por eje dominante (X en vigas horizontales, Y si la vista es vertical/rotada) ⇒ la secuencia queda izquierda→derecha como el plano. (3) Dedupe SOLO de texto superpuesto (mismo token en la misma posición): `6#6840` arriba y abajo son DOS líneas. (4) Variantes de notación: token con espacios `6 # 6 330` y `L=` explícito (`6#6 L=330` ⇒ 330 cm; `L=33` ⇒ ×10) en `compact-stirrup-notation.ts`.
- B Bandas: divisor por marcadores (centro del rango, epsilon 15%) o por el mayor salto en Y entre textos (epsilon 25% del salto); la franja intermedia queda `sin_clasificar` en el campo NUEVO `unclassifiedLongitudinalBars` (antes se metía en `topLongitudinalBars`), con warning + statusReason `requires_review`; sin separación detectable NADA se clasifica a ciegas. Estribos/zonas F8D intactos (el rescate NO aplica a zonas: la suma honesta 141 vs 140 se conserva).
- C Takeoff Dispatch Contract (`buildBeamTakeoffDispatch`, nuevo; `beamDetailToManualLines` = wrapper): para 4+4+match envía 9 líneas — 4 superior + 4 inferior (cada una `4#6330`… con cantidad del conteo GRÁFICO, `manualBarNumber`, evidencia con position/beamDetailId/capa/coords/handles/fragmento) + UNA línea de estribos al final. Regla dura (aclaración de la usuaria): al takeoff SOLO entran líneas computables (cantidad numérica + longitud + varilla) — sin cantidad gráfica confiable la barra queda en `skippedBars` ("Requiere definir cantidad"), visible en el panel, jamás inventada. Estribos: match ⇒ resumen; mismatch/unverified ⇒ solo con decisión humana (resumen del plano / cálculo por zonas); ambiguous ⇒ bloqueado siempre; zonas sueltas jamás. `previewText` ("Se enviarán 9 línea(s): 4 superior, 4 inferior, 1 estribo.") + `explanations` cuando algo queda fuera.
- D UI: panel "Ver detalle" — conteos en Resumen (Superior/Inferior N items; Estribos "1 resumen + N zonas"), secciones 2/3 con conteo, sección nueva "Sin clasificar / revisar", los 3 botones de decisión de estribos ahora FIJAN la decisión (no envían solos) y el ÚNICO "Enviar al takeoff" manda todo el dispatch con preview + motivos + barras omitidas. Listado compacto: fila envía por dispatch con `title` = preview.
- E Fixture sintético VC-EJE-3 por código (jamás DXF real): superior `6#6 33 L/6#6840/6#6900/6#635`, inferior `6#6440/6#6840/6#6730/6#6400`, 7 zonas+resumen, 4+4 marcadores; y fixture de DOS detalles contiguos en fila (retícula 2×2) que REPRODUCE la pérdida (texto a 8 unidades del código vecino ⇒ ambiguo ⇒ rescatado; el vecino no lo absorbe).
- Tests: +27 nuevos (`f8e-longitudinal-sequence.test.ts`: secuencia 4+4 en orden, normalizaciones, dedupe, sin_clasificar, rescate entre vistas, contrato de dispatch completo con las 5 reglas de estribos, líneas solo computables, evidencia, estáticos UI). Suite completa 3098/0 (42 skipped gated); typecheck 0; lint 0.
- Restricciones: sin DB/Supabase/RLS/migraciones/storage/subidas/APIs/keys/dependencias nuevas; sin archivos reales en Git; flag `STEEL_OPS_UIX_PREVIEW` intacto; sin tocar nav global/ACCESS_MODULES/NAV_ITEMS; F1 única calculadora; nada se autoaprueba; jamás se inventa cantidad.

---

## 2026-07-06 - STEEL_OPS_F8D_VIEW_SEGMENTATION_TAKEOFF_REVIEW - EN RAMA + PR - Fable

- Rama `feature/steel-ops-f8d-dxf-view-segmentation` (base `origin/main = f862273`, post F8C #67). PR `feat(steel): segment DXF beam views and refine takeoff review`. 7 problemas reales de la prueba de producción con "2504-010A R1 Refuerzo Vigas de Cimentación".
- A DXF View Segmentation (`lib/steel/dxf/dxf-view-segmentation.ts` puro, nuevo): detecta vistas/detalles/cortes INDEPENDIENTES antes de asociar textos. Dos etapas: (1) clusters geométricos single-linkage con enlace adaptativo (mediana NN ×4) usando TODOS los puntos de anclaje — el parser ahora captura aditivamente x/y de LINE/LWPOLYLINE y el punto final 11/21 de LINE; líneas larguísimas (ejes/bordes, >30% diagonal) no cosen vistas; (2) retícula de BANDAS inferida de los propios códigos de viga (eje con más bandas = apilado; paso mediano; tolerancia 0.45×paso) y SEGMENTOS por banda: el territorio de un detalle va desde su código hasta el siguiente código de la fila; asignación POR ENTIDAD (las cotas/secciones que cruzan filas ya no cosen detalles); pegado a una frontera o entre bandas ⇒ `ambiguous` (nadie lo reclama sin revisión); sin retícula ⇒ adhesión isotrópica con ratio de claridad 1.5. Cada vista: viewId/type (beam_detail/beam_longitudinal/cross_section/table/title_block/unknown)/bbox/capas/colores/handles/candidateBeamKeys/confianza/warnings; ≥2 códigos en un cluster ⇒ warning "confianza de segmentación baja". `assembleBeamDetails`/`buildBeamSchedule` filtran por vista (ambiguas y de otras vistas se excluyen con conteos visibles `crossViewExcludedCount`/`ambiguousExcludedCount`); con vista el ámbito es LA VISTA (reach ∞; una viga larga excede el radio euclidiano), sin vista se conserva el radio F8C. SMOKE real (fuera del repo, borrado): 12/12 vistas beam_detail conf 0.9; **11/12 detalles con MATCH EXACTO zonas=resumen** (140=140, 153=153, 301=301, 303=303, 256=256, 252=252…) y VC-EJE-3 con 141 vs 140 ⇒ mismatch honesto por 1 — los "1808 vs 153" eran mezcla de vistas, no errores del plano.
- C Stirrup Summary Contract (`lib/steel/dxf/stirrup-summary-contract.ts` puro, nuevo): declaredSummary/zones/zoneTotalPerRepetition/declaredPerRepetition/suggestedPerRepetition/difference/comparisonStatus (`match|mismatch|unverified|ambiguous`)/suggestedTakeoffLine (solo match)/possibleViewMixing. Envío: match ⇒ SOLO el resumen entra por defecto (jamás las zonas sueltas); mismatch ⇒ bloqueado, decisión humana (`resolveStirrupTakeoffChoice`: usar resumen del plano / usar suma por zonas / marcar para revisión); unverified ⇒ no automático (elección explícita permitida); ambiguous ⇒ no entra ni con elección + mensaje "Posible mezcla de detalles contiguos. Revisar segmentación." `beamDetailToManualLines` acepta `stirrupChoice`; nuevo `stirrupChoiceToManualLine`.
- B Listado + panel: "Listado de vigas detectadas" ahora es tabla COMPACTA (viga/eje/sección/sup/inf compactos "4×6#6330 +N"/chip de estado de estribos con zonas-vs-resumen/conf/estado/acciones) + botón "Ver detalle" que abre `beam-detail-review-panel.tsx` (panel lateral): 1 Resumen (viga/ubicación/sección/estado/confianza/vista), 2 Superior y 3 Inferior (texto original/normalizado/cantidad gráfica/fuente), 4 Estribos (zonas, subtotal, declarado, diferencia, sugerido, estado + los 3 botones de decisión humana), 5 Cálculo (fórmulas F1 legibles), 6 Evidencia CAD (archivo/capa/color/coords/handles/fragmentos/conteos de exclusión).
- D Ver cálculo (`lib/steel/steel-calculation-explanation.ts` puro, nuevo): texto original → interpretación → cantidad (piezas×repeticiones) → longitud corte → fórmula ML → fórmula KG (peso kg/ml de la varilla) → varillas comerciales → FUENTE del desperdicio → evidencia → advertencias; solo PRESENTA valores de F1, no recalcula. Botón "Ver cálculo" por línea en la tabla (fila expandible) y sección 5 del panel.
- E Desperdicio transparente: `wasteMode: calculated|manual` + `manualWastePercent` (0–30, validado, inválido ⇒ cae a calculated) en `ManualTakeoffRecord`; `effectiveWasteConfig`; `computeManualLine(s)` acepta `manualWastePct` y expone `wasteSource`; editor en el plan de corte (calculado por optimización vs factor manual, % editable); KPI y tabla declaran la fuente; el optimizador FFD intacto (su totalWasteM es el desperdicio CALCULADO). Excel: 06_CONFIGURACION filas "fuente del desperdicio"/"factor manual", notas en 00_RESUMEN (asumido/factor manual X% vs calculado por optimización).
- F Excel comercial ICONIC: headers SUAVES (azul claro FFE9F1FC + texto oscuro, acento cyan) en vez del bloque navy; banner navy-sobre-blanco; borde EXTERIOR medium azul ICONIC + líneas interiores en todas las tablas; 100% español (estados por label maps: Borrador/Disponible/Necesita revisión…, método de lectura "texto nativo PDF/OCR/DXF (CAD)", "mock"→"(referencia)", "Fecha de exportación", "Cuantificación"); fórmulas/hojas/sanitización intactas (tests las fijan).
- G PDF no obsoleto: copy en intake PDF y DXF — DXF = motor principal de lectura CAD estructurada; PDF = evidencia visual/contractual y fallback; DXF+PDF = comparación recomendada. Nada del camino PDF se eliminó; la comparación DXF↔PDF sigue cableada.
- Tests: +52 nuevos en 5 archivos (segmentación 12 con fixture de 2 detalles contiguos + franja ambigua + sin-bbox-claro; contrato estribos 15 con reglas de envío; explicación 7; desperdicio 11; estáticos UI F8D 12) + 6 archivos de tests actualizados (Excel español/estilo suave, pedido "(referencia)"). Suite completa 3071/0 (42 skipped gated); typecheck 0; lint 0.
- Restricciones: sin DB/Supabase/RLS/migraciones/storage/subidas/APIs/keys/dependencias nuevas; sin archivos reales en Git (smoke local borrado); flag `STEEL_OPS_UIX_PREVIEW` intacto; sin tocar nav global; F1 única calculadora en la app (el Excel conserva fórmulas offline por mandato); nada se autoaprueba; jamás se inventa cantidad.

---

## 2026-07-06 - STEEL_OPS_F8C_BEAM_DETAIL_ASSEMBLY - EN RAMA + PR - Fable

- Rama `feature/steel-ops-f8c-beam-detail-assembly` (base `origin/main = 68102e1`, post F8B #66). PR `feat(steel): assemble DXF beam details and compress cut-plan review`.
- A UX plan de corte (`lib/steel/cut-plan-view.ts` puro + `manual-cut-plan-section.tsx`): contenedores max-h-72 con scroll interno y sticky headers; filtros por varilla/longitud comercial/búsqueda (matchea descripción de línea) y modos todo/solo-rechazados/solo-sobrantes; grupos por varilla colapsables (default: solo el primero abierto con >3 grupos); índice compacto de navegación (#3 · #5 · Rechazados · Sobrantes) con anchors; rechazados/excluidos en listas con scroll. Los filtros SOLO cambian lo visible.
- B–F Beam Detail Assembly (`lib/steel/dxf/dxf-beam-detail-assembly.ts`): cada corte de viga como elemento compuesto — embebe la fila F8B (eje/sección) y agrega: longitudinales superior/inferior separados por banda (divisor = marcadores o mayor salto en Y), longitudes compactas normalizadas (`6#6 33 L`→6#6330, `6#635`→6#6350 con contexto; sin contexto/rango absurdo ⇒ requiresReview), CANTIDAD desde MARCADORES gráficos SHAPE-DRIVEN (CIRCLE nuevo en parser + INSERTs repetidos compactos; cluster espacial con descarte de outliers; rótulo jamás cuenta; color/capa estructural solo SUBE confianza — círculos negros cuentan igual con confianza baja; ambiguos ⇒ requires_review; el primer dígito del texto JAMÁS es cantidad), zonas de estribos agrupadas/ordenadas por X con subtotal vs resumen 2xN (mismatch por 1 con mensaje del mandato; subtotal desproporcionado >2× ⇒ unverified "vecindad mezclada", no mismatch engañoso), segmentos por simetría (reuso F8B) y evidencia completa (handles/fragmentos/coords/beamDetailId/position). `beamDetailToManualLines`: al takeoff SOLO lo verificable — resumen f1Ready y longitudinales con cantidad gráfica (`4#6330`).
- Quality Report ampliado (parámetro opcional aditivo): markersDetectedByShape/ByLayerColor, ambiguousMarkers, markerConfidence + notas.
- G UI: "Listado de vigas detectadas" ahora es el detalle compuesto (viga/eje/sección/superior/inferior/estribos con subtotal+resumen+comparación/cantidad gráfica ↑↓/capa-color/confianza/estado/acción), con scroll interno y sticky header; envío por fila via beamDetailToManualLines.
- H Excel contrato VC-VERF (hoja VC-VERF-06072026 del Excel de referencia de la usuaria; archivo NO disponible localmente — estructura tomada del mandato): 01_CANTIDADES re-diagramada ITEM/ELEMENTO ESTRUCTURAL/UBICACION EJE/DESCRIPCION/Ø/CODIGO(F, editable)/(W) VARILLA(G, VLOOKUP)/LONG COMERCIAL(H, VLOOKUP editable)/LONG CORTE(I, editable)/CANT.(J)/SON(K)/ML X UND.(L=I)/(W) X UND.(M=I*G)/CANT. ML TOTAL(N=I*J*K)/(W) TOTAL(O=N*G)/CANT. VARILLAS(P=CEILING(N/H))/columnas VARILLAS {len} M por longitud configurada (IF($H=len,$P,0))/DESPERDICIO; ELEMENTO/UBICACION desde evidence.elementKey/locationText (F8C) con fallback al alcance. Resumen suma N/O/P. Evidencia ManualLineEvidence += elementKey/locationText/beamDetailId/position (aditivo) y llega a EVIDENCIAS via observation+campos.
- Tests: +44 netos (21+1 assembly con guarda, 13 cut-plan-view+estáticos UI, resto realineación Excel); suite steel 423→ verde; suite completa ver PR. Smoke real: 12 beam details con eje/sección/sup-inf normalizados y resumen 2x153E#3184 enviable; cantidades honestas en revisión (marcadores ambiguos a escala real) y zonas mezcladas ⇒ unverified. Smoke borrado antes del commit.
- Restricciones: sin DB/Supabase/RLS/storage/subidas/APIs/keys/dependencias nuevas; sin archivos reales en Git; flag intacto; F1 única calculadora en la app; Excel con fórmulas offline por mandato; nada se auto-aprueba; jamás se inventa cantidad.

---

## 2026-07-06 - STEEL_OPS_F8B_DXF_NOTATION_BEAM_SCHEDULE - EN RAMA + PR - Fable

- Rama `feature/steel-ops-f8b-dxf-notation-beam-schedule` (base `origin/main = 671231d`, post F8A #64). PR `feat(steel): normalize DXF rebar notation and add beam schedule extraction`. 5 problemas reales del plano "2504-010A R1 Refuerzo Vigas de Cimentación".
- P1 Nomenclatura decimal compacta (`lib/steel/dxf/compact-stirrup-notation.ts`): regla explícita `E#318.4` = E#3 L=184 cm (decimal de 1 dígito ×10), solo en contexto estructural de viga/estribo y rango plausible 30–600 cm; original preservado + texto normalizado + warning "18.4 → 184 cm"; fuera de contexto o absurdo ⇒ requiresReview sin cambio silencioso. Integrada en `detectDxfNotationCandidates` (candidatos DXF llevan warning y originalFragment original). Smoke real: las 12 notaciones `2x###E#318.4` del plano ahora son f1Ready (antes ilegibles) ⇒ 2x153→306 estribos #3 de 1.84 m.
- P2 Segmentos gráficos del flejado (mismo módulo): `evaluateStirrupSegmentSum` — 50/30/12 con simetría ⇒ 50+30+12+12+30+50=184 confirmado; mismatch honesto solo con hipótesis completa ("El gráfico suma X pero la nomenclatura indica Y"); incompleto sin simetría ⇒ `graphic_segment_sum_unverified`. Cableado al beam schedule (números de cota cercanos al estribo).
- P3 Beam Schedule (`lib/steel/dxf/beam-schedule.ts` + tabla UI "Listado de vigas detectadas"): fila profesional por viga — elemento / eje-ubicación (del código VC-EJE-X o texto cercano) / sección / refuerzo longitudinal (6#655) / estribos (raw + normalizado + longitud) / observación de segmentos / capa+color / confianza / estado (`ok`/`missing_location`/`requires_review` con razones). Botón "Enviar al takeoff": SOLO tokens f1Ready entran como líneas F3 con evidencia dxf (viga/eje/sección/capa en observación); clic humano = aprobación. Smoke real: 12 filas VC-EJE-1…D con EJE, 50x60 y sus tramos de estribos.
- P4 Color/capa (`dxf-entities` +62/420, `dxf-entity-filter.ts`, UI "Filtrar entidades CAD"): colorIndex/trueColor capturados; rojo = señal (+0.05 confianza, cap 0.95, `redSignal` visible); filtro todas/solo-estructurales/custom (capas+colores) + incluir/excluir rótulo — SOLO cambia lo visible, jamás destruye datos.
...
- ACTUALIZACIÓN 2026-07-04 (misma ola, autorización explícita): compuerta `V5_6_6C_DB_APPLY_GATE` EJECUTADA contra construction-ops-prod (ref jabddbccmhrxztfzpdii). Preflight: solo las 2 migraciones V5.6.6C pendientes, cero ajenas. `db push --linked` sin errores. Post-verify por schema dump read-only: is_scoped_role con 10 políticas (projects + 9 project-chain), check `grants_only_for_scoped_roles` presente y `grants_only_for_consulta` eliminado, is_client_role intacto en los deny (93 refs), has_project_grant intacto. CONTEOS BACKFILL (data dump read-only, borrado tras verificar): prod NO tiene usuarios obra/compras hoy (perfiles: 1 admin + 1 presupuestos) ⇒ backfill = 0 filas (no-op correcto); 1 proyecto; 1 grant legacy V5.6.4 inerte (su perfil ya no es consulta; revocable vía RPC cuando se quiera). Nuevos obra/compras nacerán con 0 grants. Sigue: merge PR #43 + deploy + smoke.
...
## 2026-07-03 - STEEL_OPS_UIX_FLAG_CONTRACT - PAUSA DE IMPLEMENTACIÓN, SOLO DOCS - Fable UIX (sesión principal, sin subagente)

- Motivo: la usuaria detectó que la verificación de F1 (Codex, dominio puro) no corrió correctamente en el worktree (`npm` en vez de `pnpm` o dependencias faltantes) y pausó explícitamente toda implementación UIX hasta que Codex cierre esa verificación en verde.
- Instrucción: cero rutas, cero componentes, cero navegación, cero toque a `app/(dashboard)`, `server/access`, `module-access`, DB/RLS/Supabase/producción/`main`. Solo documentación.
- Entregable: **`docs/STEEL_OPS_UIX_FLAG_CONTRACT.md`** (SOLO DOCUMENTO, cero código). Profundiza sobre `STEEL_OPS_UIX_V1_CONTRACT.md` los 5 puntos pedidos: (1) contrato formal del flag `STEEL_OPS_UIX_PREVIEW` (condición doble env+rol, ciclo de vida temporal); (2) recorrido paso a paso de cómo se vería el módulo sin tocar el gating real (`notFound()` si no cumple condición, sin entrada de sidebar, cero cambio en `ACCESS_MODULES`/`NAV_ITEMS`/command palette); (3) propuesta de archivo responsable (`lib/steel/preview-gate.ts`, función pura, sin dependencias de `server/access/`) — propuesto, NO creado; (4) tabla explícita de qué puede hacer Fable UIX ahora (specs en prosa) vs qué espera a Codex/F1 (cualquier archivo de código); (5) spec detallada en prosa de la Rebanada UIX 1 (preview-gate, types, mocks con casos límite de D3/D5, page.tsx, dashboard, tests) — sin escribir ningún archivo.
- Nota registrada (no una objeción, solo información para la usuaria): la Rebanada UIX 1, tal como está especificada, no importa nada de `modules/steel/*` ni de tablas `steel_*`, por lo que no tiene dependencia técnica real de que F1 esté verificado — la espera actual es una decisión de secuenciación de la usuaria, respetada sin cuestionar.
- SIGUIENTE: esperar (a) aprobación de la usuaria sobre este documento y (b) confirmación de Codex de que F1 quedó verificado (typecheck/lint/tests/gm en verde con `pnpm`) antes de ejecutar cualquier código de la Rebanada 1.

---

## 2026-07-03 - STEEL_OPS_UIX_V1_CONTRACT - CONTRATO CREADO (PROPUESTO, pendiente de aprobación) - Fable UIX (sesión principal, sin subagente)

- Rama `feature/steel-ops-fable-uix` (heredada, git status limpio al iniciar). Sin commit (a la orden de la usuaria). NO se tocó DB, Supabase, RLS, `main`, ni ningún archivo compartido.
- Contexto: la usuaria confirmó que Codex ya tiene aprobación para F1 dominio puro en `feature/steel-ops-v1b-domain` (ver `docs/F1_DOMAIN_CONTRACT.md`) y redirigió el trabajo de esta sesión exclusivamente a UIX/arquitectura visual sobre `feature/steel-ops-fable-uix`, usando el blueprint como base, con mocks y feature flag, sin datos reales.
- Entregable: **`docs/STEEL_OPS_UIX_V1_CONTRACT.md`** (PROPUESTO). Mapa de pantallas (§8 del blueprint) a rutas reales `app/(dashboard)/steel/*`; tabla de reuso de componentes existentes (EmptyState, InlineCallout, StatusBadge, KpiCard, Recharts, AG Grid Community, wizard de import, review-center, link-to-boq); catálogo de estados; estructura de mocks (`lib/steel/mocks.ts`, patrón de `lib/utils/mocks/index.ts`); casos de prueba UI mínimos; lista de archivos por rebanada (rebanada 1 = hub+dashboard+empty states); riesgos; checklist.
- Hallazgo clave: **no existe mecanismo de feature flag en el repo.** El gating real es la matriz `ACCESS_MODULES`/`MODULE_ACCESS` en `apps/web/server/access/module-access.ts`, que declara explícitamente "Propiedad: agent-orchestrator" y no tiene módulo `steel`. Agregarlo (o tocar `NAV_ITEMS` en `sidebar-nav.tsx`) es una edición a archivo compartido → regla 13 de CLAUDE.md exige escalar, y esta fase NO la hace.
- Decisión de diseño (técnica, reversible, documentada en el contrato §1-2): flag interino y 100% aislado (`apps/web/lib/steel/preview-gate.ts`, env `STEEL_OPS_UIX_PREVIEW` ausente=off + rol admin/gerencia/presupuestos), rutas `steel/*` no enlazadas en el sidebar compartido (solo URL directa, 404 sin flag+rol). Satisface D1 para esta fase sin tocar `module-access.ts`/`sidebar-nav.tsx`. La integración real (agregar `steel` a `ACCESS_MODULES`, `NAV_ITEMS`, reemplazar por `STEEL_OPS_ENABLED`/`requireModuleAccess('steel')`) queda explícitamente diferida y escalada a agent-orchestrator (§9 del contrato).
- SIGUIENTE: aprobación humana del contrato → ejecutar rebanada 1 (§8: `preview-gate.ts`, `lib/steel/types.ts`, `lib/steel/mocks.ts`, `steel/page.tsx` hub+dashboard, empty states, tests). Rebanadas 2-6 (takeoffs/sources/optimize/orders/catalog+links+settings) una a una, con checklist §11 en cada una.

---

## 2026-07-03 - STEEL_OPS_F1_DOMAIN_CONTRACT - CONTRATO CREADO (PROPUESTO, pendiente de aprobación humana) - agent-orchestrator

- Rama actual `feature/steel-ops-fable-uix` (git status limpio al iniciar). NO se tocó `main`, DB, Supabase, RLS, rutas, UI, flags ni `package.json`. Solo se creó documentación + esta entrada. Sin commit (a la orden de la usuaria).
- Contexto: F0 (blueprint STEEL_OPS_V1) aprobado por la usuaria y mergeado en PR #33 (docs-only confirmado). La usuaria fijó condiciones no negociables (no duplicar precios ni jerarquía de obra; todo detrás de feature flag; nada de migraciones/RLS/Supabase/deploy/merge sin aprobación humana) y decisiones D1-D5. Encargó el CONTRATO técnico de F1 (dominio puro), NO implementación.
- Entregable: **`docs/F1_DOMAIN_CONTRACT.md`** (PROPUESTO). Cubre los 7 puntos pedidos: (1) contrato completo — objetivo/alcance/no-alcance/interfaces (parser, calculadora, optimizador FFD, banco de sobrantes, clasificador de desperdicio D5)/invariantes/costura a fuentes de verdad de precios y `project_scopes` sin duplicarlas/feature flag de cara a F3; (2) lista exacta de archivos (9 en `apps/web/modules/steel/*`, 6 tests en `apps/web/tests/unit/steel/*`, 3 fixtures en `apps/web/tests/fixtures/steel/*`); (3) casos de prueba mínimos con casos límite (umbral mínimo útil D3, cruce de umbral advertencia/crítico D5, longitudes comerciales mixtas por proveedor, kerf cero vs configurado); (4) estructura de fixtures sanitizados (D2) con ejemplos JSON inline de refuerzo y perfil; (5) riesgos R1-R9; (6) ámbito de ejecución de Codex (`feature/steel-ops-v1b-domain`, gates verdes, gm 22/22); (7) trabajo paralelo de Fable UIX (visual, flag-gated, sin depender de F1).
- Hallazgo de compatibilidad clave: **el blueprint NO contradice las condiciones 2 y 3.** El propio blueprint (§4/§14/§18) ya manda reusar `resources`+`supplier_products`+`price_observations` (sin catálogo de precios paralelo) y reusar `project_scopes` (sin jerarquía paralela). Coincide con la usuaria. Única divergencia real reportada como **R1**: el blueprint §20 pone el "evaluador de alertas" completo dentro de F1; el alcance F1 de esta tarea solo pide parser/calculadora/FFD/banco/tests. El contrato acota F1 al clasificador de desperdicio D5 y difiere las alertas A1-A18; **decisión DA-F1-1 pendiente de la usuaria** (no resuelta unilateralmente).
- Patrón arquitectónico anclado en el código real: dominio puro en `apps/web/modules/*` (no hay `packages/`), `DecimalString`+`decimal.js` (Política Q9), funciones puras sin I/O, redondeo de presentación fuera del dominio. Precios se consumen por inyección de `DecimalString` (nunca el dominio lee `PricingReadPort`); `project_scopes` se reusa como atributos opacos de paso (`projectScopeId`/`zone`/`workFront`). No existe `modules/steel` todavía.
- Decisiones pendientes de la usuaria antes de ejecutar F1: DA-F1-1 (alcance de alertas), DA-F1-2 (aprobar pesos normativos del seed `steel-specs.reference.json`, hoy PROVISIONAL), DA-F1-3 (convención de conteo por separación `floor(span/spacing)+1`).
- SIGUIENTE: aprobación humana del contrato → activar `agent-cost-domain` (rol Codex Calc) con el ámbito §12 en rama aislada `feature/steel-ops-v1b-domain`. Fable UIX puede avanzar en paralelo lo visual (§13) sin bloqueo.
...
- Rama MODELO feature/v5-6-4-client-project-scope-model (base origin/main=a399a5c): doc de contrato + OPEN_QUESTIONS (compuerta V5_6_4_DB_APPLY_GATE + deudas V5_6_5_CLIENT_RLS_FULL_CHAIN y V5_6_5_CONSULTA_WRITE_HARDENING) + DATABASE_SCHEMA + migraciones 20260702090000 (tabla project_access_grants, RPCs SECURITY DEFINER grant/revoke_project_access solo admin/gerencia y destino solo consulta, audit actions nuevas, backfill unico consulta->proyectos de su org) y 20260702090100 (RLS ENABLE+FORCE de la tabla; helper app.has_project_grant; PATCH projects_select: consulta/client requieren grant; cascada automatica a scopes/estimates/versions/chapters/boq/quantities; planning y quantity_workspace/takeoff NO cascadan = deuda V5.6.5) + harness rls-runtime seccion [GR] 18 checks.
- Validacion DB LOCAL real: supabase db reset local aplico las 2 migraciones + seeds sin errores; harness RLS 277 PASS / 0 FAIL (incluye deny-by-default, scoping intra-org, cascada, paridad claim client, JWT real solo-sub, RPCs con guards/idempotencia/audit, escrituras directas denegadas). Corregido drift preexistente del harness: FORCE count 41->43 (quick_notes no habia actualizado el conteo).
- Rama ENFORCEMENT feature/v5-6-4-client-project-scope-enforcement (apilada sobre modelo): ViewerContext.projectGrants ('all'|Uuid[]) + helper puro server/read-model/project-grants.ts (fail-closed) + choke point en drizzle-repository (visibleProjects/visibleProjectById; cadena version->estimate->scope->proyecto lanza el MISMO not-found) + espejo en fixture-repository + resolve-viewer resuelve grants RLS-bound (error de lectura => [] fail-closed, nunca fail-open) y toViewerContext acarrea profileId (claim sub para la RLS project-scoped) + exports (/api/exports pasa grants del usuario AUTENTICADO, no del perfil solicitado; /api/estimates/export check isProjectGranted 404) + matriz V5.6.2: consulta retirado de apu (decision de negocio; unica linea cambiada en MODULE_ACCESS) + UI /settings/access columna Proyectos con asignar/retirar por RPC + empty states consulta en dashboard/projects + callout en invitaciones.
- Tests nuevos: tests/unit/read-model/project-grants.test.ts (15 checks anti fail-open: helper puro, wiring fixture en TODOS los metodos que derivan proyecto, drizzle scoping intra-org con cadena de version). Ajustados: tests de proyeccion por rol usan projectGrants:'all' explicito; module-access/command-palette reflejan consulta sin apu.
- QA: typecheck 0; lint 0; suite completa 2468 passed / 42 skipped; gm:regression 22/22; build OK; git diff --check limpio.
- SEGURIDAD (no resuelto en esta fase, documentado): con publishable key + JWT, consulta puede leer PostgREST directo; la RLS project-scoped SOLO queda activa al aplicar las migraciones a Cloud (compuerta). Tablas org-directas (planning, quantity_workspace/takeoff) y write-hardening quedan para V5.6.5. NO entregar cuentas consulta a clientes externos reales hasta cerrar V5.6.5.
- SECUENCIA DE RELEASE OBLIGATORIA: (1) merge PR modelo (sin efecto runtime); (2) COMPUERTA usuaria: aplicar migraciones a Supabase Cloud (con backfill) y verificar; (3) merge PR enforcement + deploy; (4) smoke prod + validacion manual (admin, presupuestos, consulta 0/1/N grants, URL directa no asignada). Si (3) se despliega antes que (2), consulta queda fail-closed en 0 proyectos (seguro pero disruptivo).
- Restricciones respetadas: sin db push remoto, sin Vercel envs, sin DATABASE_URL, sin SMTP, sin service_role, sin snapshots/motor financiero, sin V5.7B, sin construction-ops-1rqh.
- STOP: PRs para auditoria; migraciones NO aplicadas a Cloud; esperar compuerta explicita.
...
## 2026-07-04 - STEEL_OPS_F6_PDF_INTAKE_BLUEPRINT - DOCS-ONLY EN RAMA (sin merge)

- Rama `feature/steel-ops-f6-pdf-intake-blueprint`, base `f614579` (main con F3 #41). **Fase 100% docs-only**: blueprint + contrato UX + modelo de confianza + roadmap del intake de PDF/planos para Steel Ops. Cero código, cero dependencias.
- **Regla madre de la fase:** el sistema es asistente de lectura y extracción, NO verdad automática. Candidatos con evidencia (archivo/página/zona/texto literal) + confianza; el humano corrige/aprueba/descarta; solo lo aprobado se convierte en líneas del takeoff manual F3, que recalcula TODO con el dominio F1. F6 no tiene calculadora propia (una sola fuente de verdad).
- **Docs nuevos:**
  - `docs/STEEL_OPS_F6_PDF_INTAKE_BLUEPRINT.md` — estrategia V1 (qué sí/qué no prometer), taxonomía de fuentes T1–T7 (clasificación por página, corregible), pipeline conceptual de 11 pasos, análisis de alternativas (híbrido: texto nativo + tablas + selección manual asistida ahora; OCR/calibración/visión gated después), comportamiento obligatorio cuando NO hay tabla (mensaje canónico "no hay información suficiente…"), reglas anti-alucinación F6-S1…S10, contrato de datos futuro SIN migraciones (`steel_source_pages`, `steel_detected_regions`, `steel_extraction_candidates`, `steel_candidate_reviews`, `steel_takeoff_links`; `steel_source_files` YA existe en el draft F2 y se extiende, no se duplica), relación F1/F3/F4A, riesgos R1–R9, handoff a Codex.
  - `docs/STEEL_OPS_F6_PDF_INTAKE_UX_CONTRACT.md` — pantallas P1–P10 (subir PDF, clasificación por página, visor+texto detectado con selección manual asistida, bandeja de candidatos, compare original↔interpretación, corrección con el form F3 embebido, aprobar/descartar con razón, asignar elemento/ubicación, conversión con modal resumen, registro de advertencias), estados vacíos honestos, accesibilidad. Bajo `/steel/takeoffs/[id]/sources`, tras `STEEL_OPS_UIX_PREVIEW`, sin navegación global.
  - `docs/STEEL_OPS_F6_EXTRACTION_CONFIDENCE_MODEL.md` — 5 niveles (`alta|media|baja|requiere_revision|no_interpretable`), factores explícitos (no caja negra), mapeo por reglas con topes duros (OCR/visión ⇒ tope `baja`; asunción ⇒ tope `media`; campo crítico faltante ⇒ `requiere_revision`), convivencia con confidence/alertas de F1 (se suman, nunca se restan), anti-patrones, versionado `f6-cm-1`.
  - `docs/STEEL_OPS_F6_ROADMAP.md` — F6A (upload mock + texto seleccionable + candidatos manuales) → F6B (revisión humana + conversión a takeoff) → F6C (tablas) → F6D (OCR, gate de licencia) → F6E (zonas/bbox) → F6F (calibración de escala con doble cota obligatoria) → F6G (multimodal como sugeridor, opt-in). Criterio de "lista" por sub-fase.
- **Nota de numeración:** la serie operativa `STEEL_OPS_F<n>` divergió del blueprint V1 §20 (allí "F6"=vinculación APU/BOQ+exports, "F7"=PDF con texto). Vigente: F4A=Excel export, **F6=PDF Intake**. Registrado en el blueprint F6 §encabezado.
- **Invariantes clave:** nunca inventar longitudes; nunca inferir escala sin calibración verificada (doble cota); nunca aprobar automáticamente (ni en confianza `alta`); evidencia inmutable (par detectado→corregido conservado); descarte con razón; sin totales kg/ml en F6 (solo tras conversión, calculados por F1, rotulados estimación de takeoff); F6 jamás escribe APU/BOQ/catálogo.
- **Validación:** solo docs (`git diff --stat`: 5 archivos docs). Sin suite (no hay superficie de código).
- **Restricciones respetadas:** sin DB · sin Supabase · sin RLS · sin migraciones · sin producción · sin deploy · sin `.env` · sin service role · sin navegación global · sin `ACCESS_MODULES` · sin `NAV_ITEMS` · sin tocar APU/BOQ/catálogo real · sin tocar PR #42 · sin OCR real · sin librerías nuevas · sin merge a main.
- **Siguiente paso:** revisión humana de los 4 contratos → F6A (Codex Import/Export + Fable UIX) con gate de licencia de la librería PDF antes de instalar.

---

## 2026-07-04 - STEEL_OPS_F6A_PDF_INTAKE_CANDIDATE_DETECTION - EN RAMA + PR (sin merge)

- Rama `feature/steel-ops-f6a-pdf-intake-preview` (continua sobre `22c4880`, primer esqueleto de intake). Fable lidera la deteccion de candidatos: el modulo `lib/steel/pdf-intake-preview.ts` del esqueleto queda **suplantado** (eliminado en esta misma rama, nunca llego a main) por `apps/web/lib/steel/pdf-intake-candidates.ts`, detector puro completo conforme al blueprint F6 (F6-S1..S10).
- **Detector puro (8 reglas priorizadas con supresion de solapes):** notacion compacta F1 (`5#5600`, `74E#3200`, `2X65E#3182`, `10#7205 @ 15CM`), longitud explicita (`#4 L=0.62`), segmentos (`15 + 35 + 15 = 65 cm`, con verificacion del total declarado: contradiccion => needs_review, jamas se corrige sola), lenguaje natural (`240 varillas #4 de 62 cm` => normalizado a `240#462` **con verificacion de ida y vuelta contra F1**; si F1 no reproduce la lectura, no hay conversion), especificacion de separacion (`#3 @ 0.15`, `Estribos #3 @15`, `E#3 cada 15 cm` => parcial: falta luz/cantidad), mencion de varilla (`barras longitudinales #5` => parcial solo-diametro), malla electrosoldada (baja confianza), separacion huerfana (`@15CM` => no interpretable).
- **No inventar:** todo dato faltante se reporta en `missingFields` con explicacion; los parciales y no interpretables tienen `f1Ready=false` y la compuerta `canApprovePdfIntakeCandidate` **bloquea su aprobacion** (UI deshabilita boton y opcion del select con la razon). La conversion `pdfIntakeCandidatesToManualLines` filtra `approved && f1Ready` y entrega SOLO input F3 (`originalDescription` + `assumedWastePct`): F6A no calcula nada; F1 parsea/calcula al entrar al takeoff.
- **Confianza (espejo f6-cm-1, 5 niveles):** high (F1 completo >=0.85 sin revision), medium (compacta/normalizada con asuncion; tope para needsReview del parser), low (malla sin datos), needs_review (campos criticos faltantes, contradiccion, varilla fuera de #2-#18, longitud <=0, unidad asumida), not_interpretable (sin estructura). Factores extra: sospecha OCR (letras O/l/I entre digitos => warning + degrada high y fuerza revision) y elemento asociado (+0.05 score, nunca cambia nivel).
- **Evidencia inmutable:** cada candidato conserva `evidence` (fragmento exacto, linea completa, indice de linea, pagina mock, razon de deteccion); la edicion humana (`reevaluatePdfIntakeCandidateText`) reevalua el texto canonico pero jamas toca la evidencia. Elemento/ubicacion (`VC-01`, `PILOTE P-03`, `COLUMNA C-02`) detectado por linea y asociado a sus candidatos.
- **UX (`manual-pdf-intake-section.tsx`, dentro del workspace F3, sin navegacion global):** pegar texto + pagina mock + archivo local solo-referencia (no se sube), detectar, tabla con texto editable, interpretacion, chips campos detectados/faltantes, advertencias, evidencia (pag/linea/fragmento/razon), badge de confianza con razon, estados pending/approved/discarded/needs_review, aprobar gated, enviar aprobados al takeoff. Copy obligatorio intacto: "Lectura asistida preliminar. No reemplaza revision tecnica." / "No se aprueban cantidades automaticamente. El sistema no infiere escala ni geometria en esta fase." / vacio: "No hay informacion suficiente para cantidad automatica; se requiere seleccion, calibracion o revision manual."
...
## 2026-07-04 - STEEL_OPS_F6B_PDF_TEXT_PLAN_SET - EN RAMA + PR (sin merge)

- Rama `feature/steel-ops-f6b-pdf-text-intake`, base `origin/main = 3022bfd`. F6B: PDF real con extraccion de texto seleccionable, redisenado a mitad de fase por mandato de la usuaria como **PLAN SET** (paquete de planos fuente): un takeoff real necesita planta de cimentacion (ubicacion por ejes) + cuadros + despieces + cortes/detalles, no un PDF suelto.
- **Dependencia nueva (reportada antes de instalar):** `pdfjs-dist ^6.1.200` (PDF.js de Mozilla, Apache-2.0, registrada en docs/LICENSING.md). Import dinamico SOLO en navegador al leer un archivo; alternativas descartadas: mupdf (AGPL, prohibida), pdf-parse (solo Node).
- **Extraccion client-side (`lib/steel/pdf-text-extract.ts` puro + `pdf-text-extract-client.ts` borde pdfjs):** validacion de archivo (solo PDF, tope 20 MB), texto por pagina reconstruido en lineas (agrupacion por Y con tolerancia — clave para rotulos CAD), estados pendiente/leyendo/extraido/sin_texto/error. **Caso prioritario AutoCAD:** solo capa de texto; geometria/lineas/viewports jamas; escala escrita (`ESC 1:50`) detectada como CONTEXTO por pagina (badge "solo contexto, no se usa para calcular") y nunca alimenta calculo; PDF vectorial sin texto => estado honesto `no_text` ("requiere OCR o lectura geometrica futura o revision manual") + fallback de pegado manual intacto.
- **Plan set (`lib/steel/pdf-plan-set.ts` puro):** clasificacion POR PAGINA en 6 tipos (`ubicacion_ejes|refuerzo_despiece|detalle_corte|tabla_cuadro|notas|otro`) con sugerencia por palabras clave (PLANTA/CUADRO DE/DESPIECE/DETALLE/NOTAS) siempre corregible; menciones de elemento por fuente (excluyendo EJES de grilla); `summarizeElementEvidence` agrupa evidencia por codigo de elemento (VC-01 = pareo TEXTUAL, no relacion inventada; los candidatos siguen separados por fuente) y deriva estados honestos: "Requiere vincular fuente de ubicacion (planta/ejes)" y "Requiere fuente de refuerzo/despiece"; `detectPlanSetCandidates` corre F6A por pagina incluida con ids unicos por fuente y recoge menciones de TODAS las paginas con texto.
- **Detector F6A extendido (aditivo):** evidencia += `fileName` y `sourceType`; `detectPdfIntakeCandidatesFromPages`; `extractElementMentionsFromLine` (todas las menciones por linea, sin ejes).
- **UX:** seccion "Fuentes del takeoff (plan set)": agregar N PDFs, estado por fuente, paginas con checkbox/tipo/sugerencia/caracteres/badge de escala/texto editable, quitar fuente, copiar texto extraido, "Detectar candidatos del plan set"; panel "Evidencia por elemento" con fuentes por tipo y hints de fuente faltante; tabla de candidatos muestra archivo+pagina+tipo de fuente; fallback manual intacto. Copy obligatorio completo ("Solo se extrae texto seleccionable del PDF.", "El sistema no interpreta geometria, escala ni cotas visuales en esta fase.", etc.).
- **Verificacion E2E con PDFs REALES** (generados con @react-pdf/renderer; Edge headless + CDP con carga real de archivo): plan set planta+despiece => tipos sugeridos automaticos correctos, escala 1:50 como contexto, 3 candidatos con archivo/pagina/tipo, VC-01 con evidencia doble (ubicacion+refuerzo, sin pendientes), Z-2 pidiendo fuente de refuerzo; PDF vectorial puro => "Sin texto seleccionable" + mensaje honesto. 0 errores de pagina.
- **Tests:** `pdf-text-extract.test.ts` (17) + `pdf-plan-set.test.ts` (11) + actualizaciones estaticas: no-PDF rechazado, sin_texto honesto, lineas por Y, escala jamas produce candidatos, paginas => F6A con pageNumber/fileName/sourceType, ids unicos multi-fuente, menciones sin cantidades, agrupacion textual, no-calculo en F6B, aislamiento (sin supabase/server/fetch/FormData; pdfjs solo dinamico en el borde cliente), copy obligatorio.
- Validaciones: typecheck 0 · lint 0 · steel 137/137 · build OK (worker de pdfjs resuelto por Turbopack) · suite completa **2655 passed / 42 skipped / 0 fail**.
- Restricciones: sin DB/Supabase/RLS/migraciones/storage/subida a servidor/OCR/vision/geometria/calibracion/.env/deploy/produccion/service role/navegacion global/ACCESS_MODULES/NAV_ITEMS; APU/BOQ/catalogo intactos; todo tras `STEEL_OPS_UIX_PREVIEW`.

---

## 2026-07-04 - STEEL_OPS_F6C_HYBRID_OCR_REVIEW - EN RAMA + PR (sin merge)

- Rama `feature/steel-ops-f6c-hybrid-ocr-review` (rebasada sobre `origin/main = b618410`, PR #51/#52 incluidos sin conflicto). F6C: estacion hibrida de lectura nativo+OCR, motivada por evidencia real: un PDF de AutoCAD legible a la vista omitia textos en la extraccion nativa (SHX convertido a geometria, textos rotados, cotas).
- **Dependencia OCR (reportada antes de instalar):** `tesseract.js ^7.0.0` (Apache-2.0, docs/LICENSING.md). 100% en navegador (WASM); import dinamico; la imagen jamas sale del navegador; primer uso descarga motor+modelo (~5-8 MB) desde jsDelivr (assets estaticos, sin datos del usuario); postinstall NO aprobado en pnpm (innecesario para browser). Descartadas: scribe.js-ocr (AGPL), TextDetector API (solo Chrome experimental).
- **Diagnostico de cobertura por pagina (`pdf-ocr.ts` puro):** buena/parcial/pobre/sin_texto + lineas nativas + candidatos nativos + señal `drawingOpCount` (operaciones de dibujo via pdfjs) => advertencia "texto visible convertido a geometria (SHX): usa OCR asistido" cuando hay mucha geometria y poco texto.
- **OCR por pagina elegida (`pdf-ocr-client.ts`):** pdfjs renderiza la pagina a canvas (~144 dpi) => tesseract (PSM sparse, modelo spa) => texto por lineas (misma reconstruccion por Y) + miniatura de la pagina como contexto de revision. Estados pendiente/leyendo/completado/error; fallback honesto si el motor no carga (OCR_UNAVAILABLE_MESSAGE + detalle). Nunca masivo automatico.
- **Comparacion nativo vs OCR (`compareHybridCandidates`, puro):** misma lectura por ambos => UNA fila (nativa) anotada "Confirmado tambien por OCR"; solo-OCR entra con TECHO de confianza `low`, estado needs_review y advertencia (F6-S9); mismo elemento+varilla+pagina con texto distinto => CONFLICTO marcado en ambos (fuerza revision). Stats visibles: solo nativo / solo OCR / confirmados / conflictos. Sospecha O/0, I/1, S/5 sobre la linea OCR => advertencia especifica.
- **Hallazgo real del E2E:** tesseract corrompe el simbolo `#` de la notacion (5#5600=>545600, 74E#3200=>74E:+3200). Reconstruirlo automaticamente seria inventar (posicion ambigua) => `hasLostHashSuspicion` advierte por pagina y guia la correccion humana en el textarea OCR editable. Cero invencion.
- **Evidencia por candidato:** += `method: native_text|ocr` (chip en tabla) + `crossCheck: confirmed_by_ocr|conflict`; conserva archivo/pagina/tipo de fuente/linea/fragmento como antes.
- **Fix real encontrado por E2E:** pdfjs TRANSFIERE el ArrayBuffer a su worker (queda detached) => el OCR fallaba con "Cannot perform Construct on a detached ArrayBuffer" al reusar el buffer de la extraccion. Ambos bordes cliente pasan ahora una copia (`data.slice(0)`).
- **UX estacion de revision:** por pagina: badge de cobertura + conteos + boton "Leer con OCR asistido" + estado + preview de pagina renderizada junto al texto OCR editable + advertencia de # perdido; barra de stats hibridos; copy obligatorio AutoCAD completo ("El PDF tiene texto seleccionable parcial.", "El OCR puede confundirse; revisa antes de aprobar.", "No se interpretan cotas visuales ni escala en esta fase.", etc.). Fallback manual intacto.
- **Verificacion E2E real (Edge headless + CDP, PDF generado):** cobertura visible, OCR real completado (tesseract descargo y leyo), advertencia de # perdido visible, correccion humana simulada del texto OCR => deteccion hibrida con **3 confirmados por ambos metodos** y chip "Confirmado por OCR"; camino de error de OCR tambien ejercitado. 0 errores de pagina.
