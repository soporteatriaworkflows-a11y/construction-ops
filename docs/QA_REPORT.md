...
| 2026-06-01 | orquestador (B-005 resuelto `c9e3300`, `integration/wave-3c`) | ✅ PASS | `modules/planning/types.ts` re-exporta DTOs/enums desde el read-model canónico; campo muerto `financialProgressPct` eliminado; typecheck/lint 0, 389 tests, build, gm 22/22 + 9/9, cero regresión |
| 2026-06-01 | orquestador (integración Oleada 3C exports en `integration/wave-3c` `4b904db`) | ✅ PASS pre-merge | `git merge --no-ff backup/wave3c-exports`, **sin conflictos** (15 archivos, +2200); 5 formatos (xlsx-client/internal, pdf-client/management, csv-schedule) × 4 perfiles; whitelist server-side; **0 tablas crudas**, **0 recálculo financiero** (Decimal Q9); **410 tests**; build (`/api/exports`); gm 22/22 + 9/9; validador 214/0; **smoke 6/6 HTTP 200** (PDF `%PDF`, XLSX `PK`, CSV ok) + negativos **400**; privacidad por perfil (cliente sin internos; `external_reference` solo internal); RLS runtime 32/32 vigente (sin cambios DB); **sin merge a main** |
