...
| 2026-06-14 | **OPERATIONAL_ACCESS_LAYER_V1** — tablas NUEVAS `organization_invitations` (token_hash UNIQUE, role/status CHECK, expiración, invited_by, accept/revoke, índice parcial 1 pendiente/(org,email)) y `access_audit_log` (append-only; acciones invite_created/resent/revoked/accepted/role_changed); RPCs SECURITY DEFINER `create_invitation`/`resend_invitation`/`revoke_invitation`/`peek_invitation`/`accept_invitation`/`change_member_role` (org/actor/rol server-side, anti-escalamiento); RLS ENABLE+FORCE SELECT-por-org, escrituras solo vía RPC; **no toca `profiles`**. FORCE count +2 | `20260621090000` + `20260621090100` | orchestrator |
| 2026-07-03 | **V5_6_5A_PROJECT_SCOPED_RLS_GAP_CLOSURE** — SIN tablas/columnas nuevas (FORCE count 43 sin cambio). Helper `app.is_client_role()` (`consulta`/`client`); SELECT project-scoped para roles cliente (cascada vía EXISTS a la RLS de `projects`) en planning_schedules/schedule_tasks/task_dependencies/progress_entries/resource_assignments/quantity_workspace_*/quantity_takeoff_*; SELECT denegado a cliente en 19 tablas internas (APU, catálogo, precios/descuentos, imports, quick_notes); write-hardening `NOT app.is_client_role()` en ~65 políticas (split de 6 FOR ALL: project_scopes/estimates/quantity_groups/quantity_lines/supplier_products/apu_components), inmutabilidad de emitidos intacta; trigger `profiles_guard_privileged_cols` cierra auto-escalación de `profiles.role` vía API de datos (RPCs SECURITY DEFINER exentas). Harness sección [FC] (51 checks) **328/0**. **⚠️ EN RAMA, NO aplicada a Cloud (compuerta `V5_6_5A_DB_APPLY_GATE`; requiere PR #31 desplegado antes)** | `20260702100000` + `20260702100100` | orchestrator |
| 2026-07-02 | **V5_6_4_CLIENT_PROJECT_SCOPE** — tabla NUEVA `project_access_grants` (org/profile/project FK CASCADE, `granted_by` NULL=backfill, UNIQUE `(profile_id, project_id)`, índices org/profile/project); CHECK de `access_audit_log.action` ampliado (+`project_grant_created`/`project_grant_revoked`); RPCs SECURITY DEFINER `grant_project_access`/`revoke_project_access` (solo admin/gerencia, destino solo rol `consulta`, misma org, idempotente, auditadas); helper `app.has_project_grant(uuid)` SECURITY DEFINER; **patch `projects_select`**: `consulta`/`client` solo ven proyectos con grant (deny-by-default; cascada por JOIN a scopes/estimates/versions/chapters/boq/quantities; planning y quantity_workspace/takeoff NO cascadan → deuda `V5_6_5_CLIENT_RLS_FULL_CHAIN`); backfill único consulta→proyectos de su org; RLS ENABLE+FORCE (count 41→**42**); harness sección [GR] (18 checks). **⚠️ EN RAMA, NO aplicada a Cloud (compuerta `V5_6_4_DB_APPLY_GATE`)** | `20260702090000` + `20260702090100` | orchestrator |
