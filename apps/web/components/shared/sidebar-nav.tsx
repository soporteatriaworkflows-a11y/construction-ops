...
export function SidebarNav({
  ...
  return (
    <nav className="flex-1 overflow-y-auto overflow-x-hidden py-3">
      {expanded && (
        <p className="px-4 pb-1.5 text-[10px] font-semibold uppercase tracking-wider text-iconic-soft-blue/40">
          Módulos
        </p>
      )}
      ...
      {canManageAccess && (
        <>
          {expanded && (
            <p className="mt-5 px-4 pb-1.5 text-[10px] font-semibold uppercase tracking-wider text-iconic-soft-blue/40">
              Administración
            </p>
          )}
          <ul role="list" className={cn('space-y-0.5 px-2.5', !expanded && 'mt-2 border-t border-white/10 pt-2')}>
            <NavLink {...ACCESS_ITEM} active={isActive(ACCESS_ITEM.href)} expanded={expanded} />
          </ul>
        </>
      )}
    </nav>
